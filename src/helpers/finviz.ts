...
async function addToFinvizWatchlist(symbols: string[], login: ExportLogin, watchlist: string) {
...
async function loginFinviz(driver: webdriver.ThenableWebDriver, login: ExportLogin) {
    // load page
...
async function addSymbol(driver: webdriver.ThenableWebDriver, symbol: string, row: webdriver.WebElement) {
    // reach cap
    if (!row) {
        return;
    }
