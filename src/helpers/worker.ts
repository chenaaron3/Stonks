...
import {
    conductBacktest, findIntersections,
    conductStoplossTargetOptimization, optimizeStoplossTargetForSymbol,
    conductIndicatorOptimization, optimizeIndicatorsForSymbol
} from './backtest';
import { updateStocks, gatherData, checkSplitForSymbol } from './stock';

import { RequestMessages } from '../types/job';
...
process.on('message', async (msg: RequestMessages) => {
    ...
    // thread within optimize stoploss target
        ...
        // conduct actual optimization
        let optimizedData: { [key: string]: Backtest.SymbolDataEntry[] } = {};
        let updateStep = Math.floor(symbols.length / 100);
        for (let i = 0; i < symbols.length; ++i) {
            let symbol = symbols[i];
    ...
    // thread within optimize indicators
        ...
        // conduct actual optimization
        let optimizedData: Backtest.OptimizeRoot = {};
        let updateStep = Math.floor(symbols.length / 100);
        for (let i = 0; i < symbols.length; ++i) {
            let symbol = symbols[i];
